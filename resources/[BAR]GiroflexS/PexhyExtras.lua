-- Simple Police-Lights by MuLTi! & Edsuu
...
function toggleLights(thePlayer, cmd, level)
	...
	if not(level) then outputChatBox("#FF0000 Digite /x 1 ou /x 2", thePlayer, 255, 255, 255, true) return end

	local veh = getPedOccupiedVehicle(thePlayer)
	local id = getElementModel(veh)
	if (id == 598) or (id == 596) or (id == 490) or (id == 599) then
		if(level == 1) then
...
addCommandHandler("x", toggleLights)
