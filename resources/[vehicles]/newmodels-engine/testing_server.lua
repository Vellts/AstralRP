...
addCommandHandler("ctnvehicle", function(player, cmd, arg1)
    local vehicle = tonumber(arg1)
    local x, y, z = getElementPosition(player)
    local vehicle = createVehicle(vehicle, x + 5, y + 10, z)
    if (vehicle) then
        iprint("Vehicle created")
    else
        iprint("Vehicle not created")
    end
end)