local nitroSettings = {
	...
	["percentagePerSecond"] = 5, -- bigger value: faster usage, lower value: slower usage
	["lastTick"] = 0,
...
addEventHandler("onClientVehicleStartExit", root, function(player)
	if player == localPlayer then
		...
		if nitroSettings["oldVehicle"] == nitroSettings["enteredVehicle"] then
			...
			nitroSettings["currentLevel"] = 0
			nitroSettings["enteredVehicle"] = false
		end
	end
end)
		...
		if getVehicleUpgradeOnSlot(nitroSettings["enteredVehicle"], 8) == 1010 then
			...
			if nitroSettings["nitroActivated"] then
				if nitroSettings["currentLevel"] > 0 then
					...
					if nitroSettings["currentLevel"] <= 0 then
						removeVehicleUpgrade(nitroSettings["enteredVehicle"], 1010)
						setElementData(nitroSettings["enteredVehicle"], "JOAO.Nitros", false)
					end
				else
...
addEvent("JOAO.refreshVehicleNitroLevel", true)
addEventHandler("JOAO.refreshVehicleNitroLevel", root, refreshVehicleNitroLevel)