...
function createBackFire(theVeh, scx, scy, scz)
	...
	local s = playSound3D("files/sounds/backfire2.wav", x,y,z, false)
	local s2 = playSound3D("files/sounds/123.mp3", x,y,z, false)
	setSoundMaxDistance( s, 80 )	
	setSoundVolume(s, 0.5)
	setSoundMaxDistance( s2, 80 )	
	setSoundVolume(s2, 0.3)	
end
...
function chandeMonitoringState(state,veh)
	if (state) then
		theVehicle = veh
		if not isEventHandlerAdded("onClientPreRender", root, monitorCurEngine) then addEventHandler ( "onClientPreRender", root, monitorCurEngine ) end
	else
		if isEventHandlerAdded("onClientPreRender", root, monitorCurEngine) then removeEventHandler ( "onClientPreRender", root, monitorCurEngine ) end
		theVehicle=nil
		theTestVehicle=nil
	end
end
...
function monitorCurEngine()
	if theVehicle and isElement(theVehicle) then
		if (getElementData(theVehicle, "JOAO.noiseVeh") or false) then
