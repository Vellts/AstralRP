...
addEventHandler("onClientResourceStart", resourceRoot, function()
	for _, vehicle in pairs(getElementsByType("vehicle")) do
		if isElementStreamedIn(vehicle) then
			if (getElementData(vehicle, "JOAO.Lsddoor") or false) then
...
addEventHandler("onClientElementStreamIn", root, function()
	if isVehicle(source) then
		if getElementData(source, "JOAO.Lsddoor") then
...
addEventHandler("onClientElementDataChange", root, function(data)
	if isVehicle(source) then
		if data == "JOAO.Lsddoor" then
			if isElementStreamedIn(source) then
				vehiclesWithScissorDoor[source] = getElementData(source, "JOAO.Lsddoor")
...
function setVehicleDoorToLSD(vehicle, state)
	if isVehicle(vehicle) then
		setElementData(vehicle, "JOAO.Lsddoor", state, true)
