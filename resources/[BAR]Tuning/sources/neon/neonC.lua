...
addEventHandler("onClientResourceStart", resourceRoot, function()
	...
	for _, vehicle in ipairs(getElementsByType("vehicle", root, true)) do
		local elementData = (getElementData(vehicle, "JOAO.Neon") or false)
		if elementData and elementData.tuningTable then
			if getElementData(vehicle, "vehicle.neon.active") then
				addNeon(vehicle, elementData.tuningTable, true)
			end
		end
	end
end)
...
addCommandHandler("neon", function()
	...
	if vehicle then
		if getVehicleOccupant(vehicle, 0) == localPlayer then
			local neonColor = getElementData(vehicle, "JOAO.Neon") or false
			if neonColor and neonColor.tuningTable then
				local neonActive = getElementData(vehicle, "vehicle.neon.active") or false
				iprint(neonColor, neonColor.tuningTable, neonActive)
				
				if not neonActive then
					triggerServerEvent("tuning->Neon", localPlayer, vehicle, neonColor.tuningTable)
...
--			local neonColor = getElementData(source, "JOAO.Neon") or false
--			
--			if neonColor and neonColor.tuningTable then
--				addNeon(source, neonColor.tuningTable, true)
--			end
--		end
--	end
--end)
...
addEventHandler("onClientRender", root, function()
	for vehicle, neon in pairs(vehicleNeon) do
		if neon["object.1"] and neon["object.2"] and isElement(neon["object.1"]) and isElement(neon["object.2"]) then
...
function addNeon(vehicle, neon, setDefault)
	...
	if vehicleNeon[vehicle]["object.1"] or vehicleNeon[vehicle]["object.2"] then
		...
		setElementAlpha(vehicleNeon[vehicle]["object.1"], 0)
		setElementAlpha(vehicleNeon[vehicle]["object.2"], 0)
		...
		vehicleNeon[vehicle]["object.zOffset"] = vehicleZ
	end
...
function removeNeon(vehicle, previewMode)
	...
	if not previewMode then
		setElementData(vehicle, "JOAO.Neon", false)
		setElementData(vehicle, "vehicle.neon.active", false)
	end
end

function saveNeon(vehicle, neon)
	setElementData(vehicle, "JOAO.Neon", neon)
	setElementData(vehicle, "vehicle.neon.active", false)
