...
local function renderELS()
	...
	for veh in pairs(tblRenderedELSVehs) do
		if isVehicle(veh) then
			local light = tblELSVehs[veh]
			local iVehX, iVehY, iVehZ = getElementPosition(veh)
			local iScreenVehX, iScreenVehY = getScreenFromWorldPosition(iVehX, iVehY, iVehZ, 400) -- 100 for 100px tolerance
						...
						for i=1, #light.lights do
							...
							if iCurPhase > data.start and iCurPhase < data.stop then
								...
								if uLightImage then
									...
									--draw a light source
									if exports["dynamic_lighting"] and not data.noshine then
										if not tblLightCache[veh] then tblLightCache[veh] = {} end
								...
								if exports["dynamic_lighting"] then
									if tblLightCache[veh] and tblLightCache[veh][i] then
...
local function changeVehicleELSState(bOn, uVeh)
	...
	if hasVehicleELS(source) then
		...
		if bOn then -- if the lights should be on
			if isElementStreamedIn(source) then --vehicle is streamed in -> render lights
				tblRenderedELSVehs[source] = true
			end
		else -- the lights should be off
			if tblLightCache[source] then
...
local function addVehicleELS(uVeh, tblLights)
	if not tblELSVehs[uVeh] then
		...
		addEventHandler("onClientElementDestroy", uVeh, function() removeVehicleELS(uVeh) end)
		if tblLights.bOn then
			changeVehicleELSState(true, uVeh)
		end
	end
end
...
function InitLighting()
	...
	if vehicle and getPedOccupiedVehicleSeat(localPlayer) == 0 then
		if hasVehicleELS(vehicle)  then
			local bCurrent = tblELSVehs[vehicle].bOn
			triggerServerEvent("ELS:onVehicleStateChange", vehicle, not bCurrent)
		elseif getVehicleELSPresets(vehicle) then
			triggerServerEvent("ELS:onVehicleAddByPreset", resourceRoot, vehicle, true)
		end
	end
end
